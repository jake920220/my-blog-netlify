...
## 평소에 잘 작성하지 않는 코딩도 요청하기 좋다

정규표현식 처럼 자주 사용하지는 않지만 꼭 필요한 순간이 오는, 머리로 외우고 있지 않지만 그 때 그 때 검색해서 어떻게든 쓰는 것들이 가끔 있다.

그런 경우 구글에 검색해서 어떻게든 긁어와 사용을 하곤 하지만 ChatGPT를 통해 요청하면 보다 간단하게 코드를 얻을 수 있다.

![request regexp](./images/reg.png)
> ChatGPT에게 정규식을 요청했다.

ChatGPT는 정규식을 던져주기만 할 뿐만 아니라 각 정규식이 어떤 역할을 하는지, 문법적인 설명까지 곁들여 주어 굉장히 마음에 들었다.

![request regexp test](./images/reg-test.png)

정규식을 테스트하는 코드도 짜달라하면 즉석에서 짜준다. : )
